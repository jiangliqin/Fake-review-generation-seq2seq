...
def main(args):
    ...
    train_model = Word2SeqModel(
        ...
        l2_regularize=l2_regularize, learning_rate=learning_rate, max_to_keep=max_checkpoints, state_pass=state_pass, max_topic_num=5)
    ...
    dev_writer = tf.summary.FileWriter(tf_board + 'dev/', sess.graph)
    ...
    try:
        ...
        print("Something went wrong while restoring checkpoint. "
              "Training is terminated to avoid the overscpwriting.")
        raise
    ...
    try:
        for step in range(saved_global_step + 1, num_steps):
            ...
            batch = get_train_batch(train_set, max_length, batch_size, max_src_num=5)
            ...
            if step % print_every == 0 and step != 0:
                ...
                if dev_set is not None:
                    dev_batch = get_train_batch(dev_set, max_length, batch_size, max_src_num=5)

                    dev_perp = train_model.compute_perplexity(sess, dev_batch)
                    add_summary(dev_writer, step, 'dev perplexity', dev_perp)
                    dev_str += "val_prep: {:.3f}\n".format(dev_perp)
