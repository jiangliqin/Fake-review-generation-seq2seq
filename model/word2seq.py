...
from model.cell import create_rnn_cell
...
class Word2SeqModel():
    def __init__(self, mode,
                       ...
                       l2_regularize, learning_rate, max_to_keep=100, max_gradient_norm=5.0, state_pass=False):
    ...
    def build_model(self):
        ...
        with tf.variable_scope('seq2seq_encoder'):
            embed_inputs = tf.nn.embedding_lookup(self.embedding, self.encoder_inputs)
            enc_mask = tf.sequence_mask(self.encoder_length, tf.shape(embed_inputs)[1])
            enc_mask = tf.expand_dims(enc_mask, -1)
            enc_mask = tf.tile(enc_mask, [1, 1, self.embedding_size])
            paddings = tf.ones_like(embed_inputs) * 0.
            embed_inputs = tf.where(enc_mask, embed_inputs, paddings)
            encoder_outputs = embed_inputs
    ...
    def train(self, sess, batch):
        ...
        # res = sess.run([self.train_op, self.loss, self.enc_mask, self.encoder_outputs], feed_dict=feed_dict)
